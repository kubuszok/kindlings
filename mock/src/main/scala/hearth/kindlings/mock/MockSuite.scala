...
/** Mix-in for test suites that provides an ambient [[MockContext]] plus ScalaMock-style scoped auto-verification.
  ...
  * the bare [[MockContext]] API). The implicit [[mockContext]] is exactly the one the `Mock.mock`/`Mock.stub` macros
  * and the `(m.method _).expects/.when/.verify` DSL resolve at the call site.
  *
...
trait MockSuite {
  ...
  implicit protected val mockContext: MockContext = new MockContext
