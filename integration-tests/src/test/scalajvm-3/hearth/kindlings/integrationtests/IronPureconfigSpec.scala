...
final class IronPureconfigSpec extends MacroSuite {
  ...
  group("Iron + PureConfig") {
    ...
    test("round-trip") {
      val reader: ConfigReader[IronPerson] = KindlingsConfigReader.derived[IronPerson]
      val writer: ConfigWriter[IronPerson] = KindlingsConfigWriter.derived[IronPerson]
