...
final private[tapiropenapijsoniter] case class DecodingFailure(message: String)
...
/** Minimal circe-style `Decoder` for the kindlings [[Json]] AST.
  ...
  */
private[tapiropenapijsoniter] trait Decoder[A] { self =>
...
private[tapiropenapijsoniter] object Decoder {
...
private[tapiropenapijsoniter] trait KeyDecoder[A] { self =>
  def apply(key: String): Option[A]
  final def map[B](f: A => B): KeyDecoder[B] = (key: String) => self.apply(key).map(f)
}

private[tapiropenapijsoniter] object KeyDecoder {
...
/** A focused JSON cursor mirroring the bits of circe's `HCursor` the apispec decoders use: `get`, `getOrElse`, `as`,
  ...
  */
final private[tapiropenapijsoniter] class Cursor(val focus: Json) {
