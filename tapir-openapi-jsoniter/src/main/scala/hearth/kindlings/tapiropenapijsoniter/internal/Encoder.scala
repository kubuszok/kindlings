...
/** Minimal circe-style `Encoder` for the kindlings [[Json]] AST.
  ...
  */
private[tapiropenapijsoniter] trait Encoder[A] { self =>
  def apply(a: A): Json
  final def contramap[B](f: B => A): Encoder[B] = (b: B) => self.apply(f(b))
}

private[tapiropenapijsoniter] object Encoder {
...
private[tapiropenapijsoniter] trait KeyEncoder[A] { self =>
  def apply(a: A): String
  final def contramap[B](f: B => A): KeyEncoder[B] = (b: B) => self.apply(f(b))
}

private[tapiropenapijsoniter] object KeyEncoder {
...
private[tapiropenapijsoniter] object JsonNumbers {
