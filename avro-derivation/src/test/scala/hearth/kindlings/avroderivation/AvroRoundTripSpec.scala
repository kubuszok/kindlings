...
final class AvroRoundTripSpec extends MacroSuite {
  ...
  group("AvroIO round-trip") {
    ...
    group("Seq[Byte], List[Byte], Vector[Byte] as BYTES") {
      ...
      test("WithByteSeq binary round-trip") {
        val encoder: AvroEncoder[WithByteSeq] = AvroEncoder.derived[WithByteSeq]
        val decoder: AvroDecoder[WithByteSeq] = AvroDecoder.derived[WithByteSeq]
        val original = WithByteSeq(Seq[Byte](1, 2, 3, 4))
        val bytes = AvroIO.toBinary(original)(encoder)
        val decoded = AvroIO.fromBinary[WithByteSeq](bytes)(decoder)
        decoded.data.toList ==> original.data.toList
      }

      test("WithByteList binary round-trip") {
        val encoder: AvroEncoder[WithByteList] = AvroEncoder.derived[WithByteList]
        val decoder: AvroDecoder[WithByteList] = AvroDecoder.derived[WithByteList]
        val original = WithByteList(List[Byte](10, 20, 30))
        val bytes = AvroIO.toBinary(original)(encoder)
        val decoded = AvroIO.fromBinary[WithByteList](bytes)(decoder)
        decoded.data ==> original.data
      }

      test("WithByteVector binary round-trip") {
        val encoder: AvroEncoder[WithByteVector] = AvroEncoder.derived[WithByteVector]
        val decoder: AvroDecoder[WithByteVector] = AvroDecoder.derived[WithByteVector]
        val original = WithByteVector(Vector[Byte](5, 10, 15, 20, 25))
        val bytes = AvroIO.toBinary(original)(encoder)
        val decoded = AvroIO.fromBinary[WithByteVector](bytes)(decoder)
        decoded.data ==> original.data
      }

      test("empty Seq[Byte] round-trip") {
        val encoder: AvroEncoder[WithByteSeq] = AvroEncoder.derived[WithByteSeq]
        val decoder: AvroDecoder[WithByteSeq] = AvroDecoder.derived[WithByteSeq]
        val original = WithByteSeq(Seq.empty[Byte])
        val bytes = AvroIO.toBinary(original)(encoder)
        val decoded = AvroIO.fromBinary[WithByteSeq](bytes)(decoder)
        decoded.data.toList ==> original.data.toList
      }

      test("mutually recursive types (MutRec1 -> MutRec2 -> MutRec1)") {
        val encoder1: AvroEncoder[MutRec1] = AvroEncoder.derived[MutRec1]
        val decoder1: AvroDecoder[MutRec1] = AvroDecoder.derived[MutRec1]
        val original = MutRec1("hello", Some(MutRec2(42, Some(MutRec1("nested", None)))))
        val bytes = AvroIO.toBinary(original)(encoder1)
        val decoded = AvroIO.fromBinary[MutRec1](bytes)(decoder1)
        decoded ==> original
      }

      test("OffsetDateTime round-trip") {
        val encoder: AvroEncoder[WithOffsetDateTime] = AvroEncoder.derived[WithOffsetDateTime]
        val decoder: AvroDecoder[WithOffsetDateTime] = AvroDecoder.derived[WithOffsetDateTime]
        val ts = java.time.OffsetDateTime.of(2024, 6, 15, 10, 30, 0, 0, java.time.ZoneOffset.ofHours(5))
        val original = WithOffsetDateTime(ts, "test")
        val bytes = AvroIO.toBinary(original)(encoder)
        val decoded = AvroIO.fromBinary[WithOffsetDateTime](bytes)(decoder)
        assertEquals(decoded.label, original.label)
        assertEquals(decoded.timestamp.toInstant, original.timestamp.toInstant)
      }

      test("mutually recursive types — both None terminators") {
        val encoder1: AvroEncoder[MutRec1] = AvroEncoder.derived[MutRec1]
        val decoder1: AvroDecoder[MutRec1] = AvroDecoder.derived[MutRec1]
        val original = MutRec1("leaf", None)
        val bytes = AvroIO.toBinary(original)(encoder1)
        val decoded = AvroIO.fromBinary[MutRec1](bytes)(decoder1)
        decoded ==> original
      }
