...
/** Adds a custom key-value property to the Avro schema. Can be applied to both types (record-level) and fields.
  * Multiple `@avroProp` annotations can be stacked on the same target.
  *
