...
import hearth.kindlings.sconfigderivation.{ConfigWriter, CoproductHint, ProductHint, SConfig}
...
trait WriterMacrosImpl
  ...
  def deriveWriterTypeClass[A: Type](configExpr: Expr[SConfig]): Expr[ConfigWriter[A]] = {
    ...
    implicit val SConfigT: Type[SConfig] = WTypes.SConfig
    val selfType: Option[??] = Some(Type[A].as_??)

    deriveWriterFromCtxAndAdaptForEntrypoint[A, ConfigWriter[A]]("ConfigWriter.derived") { fromCtx =>
      ValDefs.createVal[SConfig](configExpr).use { configVal =>
        Expr.quote {
          val cfg = Expr.splice(configVal)
          hearth.kindlings.sconfigderivation.internal.runtime.SconfigDerivationFactories.writerInstance[A] { (a: A) =>
            val _ = a
            Expr.splice {
  ...
  def deriveWriterFromCtxAndAdaptForEntrypoint[A: Type, Out: Type](macroName: String)(
    ...
    Log
      .namedScope(s"Deriving writer for ${Type[A].prettyPrint} at: ${Environment.currentPosition.prettyPrint}") {
        MIO.scoped { runSafe =>
  ...
  def shouldWeLogWriterDerivation: Boolean = {
    implicit val LogDerivationT: Type[ConfigWriter.LogDerivation] = WTypes.WriterLogDerivation
    def logDerivationImported = Expr.summonImplicit[ConfigWriter.LogDerivation].isDefined
  ...
  final case class WriterCtx[A](
    ...
    def getHelper[B: Type]: MIO[Option[(Expr[B], Expr[SConfig]) => Expr[ConfigValue]]] = {
      implicit val ConfigValueT: Type[ConfigValue] = WTypes.ConfigValue
      implicit val SConfigT: Type[SConfig] = WTypes.SConfig
      cache.get2Ary[B, SConfig, ConfigValue]("cached-write-method")
    }

    def setHelper[B: Type](
        helper: (Expr[B], Expr[SConfig]) => MIO[Expr[ConfigValue]]
    ): MIO[Unit] = {
      implicit val ConfigValueT: Type[ConfigValue] = WTypes.ConfigValue
      implicit val SConfigT: Type[SConfig] = WTypes.SConfig
      val defBuilder =
        ValDefBuilder.ofDef2[B, SConfig, ConfigValue](s"write_${Type[B].shortName}")
      for {
