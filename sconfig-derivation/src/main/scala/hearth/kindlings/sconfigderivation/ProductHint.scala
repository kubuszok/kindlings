...
/** Per-type override of the global [[SConfig]] config. The macro looks for an implicit `ProductHint[A]` for every case
  * class it derives; when present, it overrides the global config's `transformMemberNames` / `useDefaults` /
  * `allowUnknownKeys` for that specific type. Annotations (`@configKey`, `@transientField`) always win.
  *
  * Defaults match the global [[SConfig]] (which themselves match upstream PureConfig's `ProductHint` defaults):
  *
  ...
  * fall back to the global [[SConfig]] config.
  */
