...
/** Compile-time evidence that `_.xs.each` type-checks, with the type member `Elem` set to the traversed element type.
  *
  * It is **materialized by a `transparent inline given`** that consults Hearth's `IsCollection`/`IsMap`/`IsOption` SPI:
  * `.each` type-checks ONLY when the container actually has a provider (the built-ins, or anything on the classpath
  * such as cats `NonEmpty*` via `kindlings-cats-integration`), and `Elem` is taken from that provider. There are no
...
object IsElementOf {
  ...
  transparent inline given derived[C]: IsElementOf[C] =
    ${ internal.compiletime.ModifyMacros.isElementOfImpl[C] }
}
