...
// `transparent inline given` that consults Hearth's std SPI and computes the type member(s), so the step type-checks
// ONLY on a real provider-backed container, with the precise types taken from it.
...
object IsIndexedElementOf {
  ...
  transparent inline given derived[C]: IsIndexedElementOf[C] =
    ${ internal.compiletime.ModifyMacros.isIndexedElementOfImpl[C] }
}
...
object IsSingleElementOf {
  ...
  transparent inline given derived[C]: IsSingleElementOf[C] =
    ${ internal.compiletime.ModifyMacros.isSingleElementOfImpl[C] }
}
...
object IsEither {
  ...
  transparent inline given derived[C]: IsEither[C] =
    ${ internal.compiletime.ModifyMacros.isEitherImpl[C] }
}
