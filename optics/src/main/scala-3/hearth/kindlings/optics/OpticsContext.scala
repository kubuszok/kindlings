...
/** Phantom capability that gates the `modify` path markers (`.each`/`.eachWhere`, `.at`/`.index`/`.atOrElse`,
  ...
  * exactly as on Scala 2. `OpticsContext extends PathStepEvidence`, so the synthesized `using OpticsContext` argument
  * is stripped by the same `isSubtypeOf[_, PathStepEvidence]` check that already drops the `IsElementOf`/… evidences.
  *
