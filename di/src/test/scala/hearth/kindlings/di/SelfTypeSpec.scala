...
/** Self-type providers (`this: AProvider =>`) and by-name (`=> A`) constructor parameters are both wired on Scala 2 AND
  * Scala 3 as of Hearth 0.3.1-48 (self-type members are exposed on Scala 2; `Parameter.byNameUnderlying` recovers the
  * by-name underlying on Scala 3). Previously each worked on only one platform; because both now work everywhere, this
  * is a single shared spec (no platform split).
  */
