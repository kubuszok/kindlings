...
private[di] trait DICompanionCompat {
  ...
  /** Build a complete object graph for `A` from an explicit list of dependencies (instances and factory functions),
    ...
    */
  inline def autowire[A](inline dependencies: Any*): A = ${
  ...
  inline def wireWith[RES](inline factory: () => RES): RES = ${
    internal.compiletime.WiringMacros.wireWithImpl[RES]('factory)
  }
  ...
  ): RES = ${ internal.compiletime.WiringMacros.wireWithImpl[RES]('factory) }
