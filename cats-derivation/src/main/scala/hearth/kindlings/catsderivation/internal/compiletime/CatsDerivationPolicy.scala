...
/** Module-level wiring for the derivation policy (issue kubuszok/kindlings#85). All three members are concrete (the
  * type-class name is generic) so the trait can be mixed into the heavily-composed cats impl hierarchy
  * (e.g. `Monoid extends Semigroup`, `Hash extends Eq`) without override/diamond conflicts.
  */
