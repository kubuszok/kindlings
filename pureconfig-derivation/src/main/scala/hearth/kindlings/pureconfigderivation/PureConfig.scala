...
/** Compile-time configuration knobs that drive kindlings PureConfig derivation.
  *
  * Defaults match upstream `pureconfig.generic.ProductHint` exactly so users moving
  * between PureConfig's own derivation and kindlings get identical default behaviour:
  *
  ...
  *    (matching upstream `FieldCoproductHint.defaultMapping`).
  ...
  *    when the corresponding HOCON key is missing (matching upstream `useDefaultArgs = true`).
  *  - `allowUnknownKeys = true` — HOCON objects may contain keys that aren't fields of
  *    the case class. Set to `false` for strict mode (fail on unknown keys), matching
  *    upstream `allowUnknownKeys = false`.
  *
  * For per-type customization (e.g. one case class wants `SnakeCase` while everything
  * else stays kebab-case), define an implicit [[KindlingsProductHint]] /
  * [[KindlingsCoproductHint]] for the specific type. The macro looks them up at
  * derivation time and falls back to this global config when no per-type hint is in
  * scope.
  */
