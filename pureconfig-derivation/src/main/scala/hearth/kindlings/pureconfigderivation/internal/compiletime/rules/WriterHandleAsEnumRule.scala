...
trait WriterHandleAsEnumRuleImpl {
  ...
  object WriterHandleAsEnumRule extends WriterDerivationRule("handle as enum when possible") {
    ...
    private def encodeEnumCases[A: WriterCtx](
      ...
      implicit val FieldHintT: Type[KindlingsCoproductHint.Field[A]] = WTypes.fieldCoproductHintType[A]
      implicit val WrappedHintT: Type[KindlingsCoproductHint.Wrapped[A]] = WTypes.wrappedCoproductHintType[A]

      // Per-type override: see ReaderHandleAsEnumRule for the same pattern.
          ...
          Log.namedScope(s"Writing enum case ${enumCaseValue.prettyPrint}: ${EnumCase.prettyPrint}") {
            val caseValueMIO: MIO[Expr[ConfigValue]] =
              if (isEnumerationOrJavaEnum)
                MIO.pure(Expr.quote(PureConfigDerivationUtils.emptyConfigObject))
              ...
              if (allCaseObjects) {
                Expr.quote {
                  ...
                  Expr.splice(discrFieldExpr) match {
                    ...
                    case None =>
                      PureConfigDerivationUtils.writeEnumAsString(name)
                  }
                }
              } else {
                Expr.quote {
                  ...
                  Expr.splice(discrFieldExpr) match {
                    ...
                    case None =>
                      PureConfigDerivationUtils.wrapWithTypeName(name, Expr.splice(caseValue))
                  }
                }
              }
            }
