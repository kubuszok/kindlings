  ...
  group("KindlingsConfigReader") {
    ...
    group("combinatorial: wrapper x inner type") {
      ...
      test("read CombOuter with all fields populated") {
        val r = KindlingsConfigReader.derived[CombOuter]
        r.from(cursor("""{
          opt-primitive = 42,
    ...
    group("annotation x type shape") {
      ...
      test("@configKey on a sealed trait subtype field (reader)") {
        val r = KindlingsConfigReader.derived[AnnotatedShape]
        r.from(cursor("{ type = annotated-circle, r = 2.5 }")) ==> Right(AnnotatedCircle(2.5))
      }

      test("@configKey on multiple fields of a sealed trait subtype (reader)") {
        val r = KindlingsConfigReader.derived[AnnotatedShape]
        r.from(cursor("{ type = annotated-rect, w = 4.0, h = 5.0 }")) ==> Right(AnnotatedRect(4.0, 5.0))
      }

      test("@transientField on a sealed trait subtype (reader)") {
        val r = KindlingsConfigReader.derived[TransientShape]
        r.from(cursor("{ type = transient-circle, radius = 3.0 }")) ==> Right(TransientCircle(3.0))
      }

      test("@transientField on a sealed trait subtype with extra fields (reader)") {
        val r = KindlingsConfigReader.derived[TransientShape]
        r.from(cursor("{ type = transient-rect, width = 6.0, height = 7.0 }")) ==> Right(TransientRect(6.0, 7.0))
      }
      ...
      test("round-trip AnnotatedShape via reader + writer") {
        ...
        r.from(pureconfig.ConfigCursor(w.to(circle), Nil)) ==> Right(circle)
        r.from(pureconfig.ConfigCursor(w.to(rect), Nil)) ==> Right(rect)
      }
