  ...
  group("KindlingsXmlDecoder") {
    ...
    group("case classes") {
      ...
      test("simple case class") {
        val decoder = KindlingsXmlDecoder.derived[SimplePerson]
      ...
      test("empty case class") {
        val decoder = KindlingsXmlDecoder.derived[EmptyClass]
      ...
      test("single field case class") {
        val decoder = KindlingsXmlDecoder.derived[SingleField]
      ...
      test("nested case class") {
        val decoder = KindlingsXmlDecoder.derived[PersonWithAddress]
        val elem = parseXml(
    ...
    group("annotations") {
      ...
      test("@xmlName renames field") {
        val decoder = KindlingsXmlDecoder.derived[XmlWithFieldName]
      ...
      test("@transientField uses default") {
        val decoder = KindlingsXmlDecoder.derived[XmlWithTransient]
    ...
    group("generic types") {
      ...
      test("Pair[String, Int]") {
        val decoder = KindlingsXmlDecoder.derived[Pair[String, Int]]
    ...
    group("deeply nested") {
      ...
      test("3 levels deep") {
        val decoder = KindlingsXmlDecoder.derived[PersonFull]
        val elem = parseXml(
